...
Yii::import('application.lib.RForm.*');

class BandController extends MusoController
{
	/**
	...
	public function filters() {
		return array(
            ...
            array('application.filters.GroupFilter + update'),
        );
	}
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Group']))
		{
			...
			$group->type = "Band";
			if($group->save()) {
				$band->group_id = $group->id;
				$band->label = $label->id;
				if($band->save()) {
					$band->addMember();
					$this->redirect(array('view','id'=>$band->id));
				}
			}
	}

		$this->render('create', array('group'=>$group,'band'=>$band,'label'=>$label));
	}
	...
	public function actionUpdate($id)
	{
		...
		$this->render('update',array('band'=>$band));
	}
