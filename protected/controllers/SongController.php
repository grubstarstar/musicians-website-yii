	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Song']))
		{
			...
			if(Yii::app()->request->isAjaxRequest) {
				...
				if($model->save()) {
					Yii::app()->user->setFlash('success', 'Your song has been saved');
				} else {
					$errors = CHTML::errorSummary($model);
					Yii::app()->user->setFlash('error', $errors);
				}
				$type = $model->group->type;
				$this->redirect(array("$type/view",'id'=>$model->group->SubGroup->id));
			}
		} 
	...
	public function actionDelete($id)
	{
		if(Yii::app()->request->isPostRequest)
		{
			...
			if($model->delete()) {
				...
				throw new CHttpException(500,'Intenral Server Error. Unable to delete this song.');
			}
		}
		else
