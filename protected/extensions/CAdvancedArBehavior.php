...
class CAdvancedArbehavior extends CActiveRecordBehavior
{
	...
	protected function writeManyManyTables() 
	{
		...
		foreach($this->getRelations() as $relation) 
		{
			...
			$foreign_ids;
			foreach($this->owner->$relation_name as $foreignobject) {
				push($foreign_ids, $foreignobject->$relation['m2mForeignField']);
			}
			print "Before CAdvancedArbehavior::cleanRelation</br>";
			printf("%s contains: %s", relation_name, implode(', ', $foreign_ids));
			$this->cleanRelation($relation);

			foreach($this->owner->$relation_name as $foreignobject) {
				push($foreign_ids, $foreignobject->$relation['m2mForeignField']);
			}
			print "Before CAdvancedArbehavior::writeRelation</br>";
			printf("%s contains: %s", relation_name, implode(', ', $foreign_ids));
			...
			print "After CAdvancedArbehavior::writeRelation</br>";
			printf("%s contains: %s", relation_name, implode(', ', $foreign_ids));
		}
	}
